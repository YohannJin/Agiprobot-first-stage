...
// copy pasted from "https://stackoverflow.com/questions/25444449/how-do-i-convert-a-stdstring-containing-doubles-to-a-vector-of-doubles"
...
int main(int argc, char **argv)
{
    ...
    std::vector<double> goalPose = getVertexIndices(inputGoalPose); // detour because getParam() with vector doesn't seem to work although defined
    if(goalPose.size() == 6)
    {
        for(int i = 0; i < 6; i++)
        {
    ...
    else if(inputGoalPose == "HOME" || inputGoalPose == "CLAMPDEVICE" || inputGoalPose == "TOOLCHANGER")
    {
    ...
    {
        ROS_ERROR_STREAM("acceleration value invalid: must be greater than 0 but is " << inputAcceleration);
        deleteParam(&nh);
        return 1;
    }
