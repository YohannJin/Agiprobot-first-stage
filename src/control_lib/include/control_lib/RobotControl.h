...
//ur_rtde
#include <control_lib/ur_rtde/rtde_control_interface.h>
#include <control_lib/ur_rtde/rtde_receive_interface.h>

using namespace ur_rtde;


enum Forcetype {WRENCH_UP, WRENCH_DOWN, GETTOOLIN, GETTOOLOUT, EJECTTOOLIN, EJECTTOOLOUT};
enum Goal {HOME, CLAMPDEVICE, TOOLCHANGER};
...
public:
	...
	std::vector<double> clampdevice_position_ = {-0.139727, -0.530038, 0.363432, 1.21277, -1.20451, -1.1598};
	std::vector<double> toolchanger_position_ = {-0.500547, 0.611285, 0.323413, -0.0620725, -2.21022, -2.21479};
   ...
   * @param useMoveit true = use moveit to calculate trajectory, false = linear movement, without collision detection but more accuracy!!!
   ...
   */
	...
	/**
   * @brief Moves robot to defined goal.
   * @param goal 6D-Vecotor with position in x,y,z coordinates and orientation in axis angles, both respective to base frame.
   * @param useMoveit true = use moveit to calculate trajectory, false = linear movement, without collision detection but more accuracy!!!
   * @param velocity Velocity of TCP.
   * @param acceleration Acceleration of TCP.
   */
	void gotoTP(std::vector<double> goal, bool useMoveit, double velocity, double acceleration);

	/**
   * @brief Calculates the distance between the x,z plane of the toolframe and a given point.
   * @param relativeTCPPose 6D-Vecotor with position in x,y,z coordinates and orientation in axis angles, both respective to base frame.
   ...
   * @param repeatHowMany number of times to execute procedure: forcemode -> back to normal mode after key was pressed.
   */
