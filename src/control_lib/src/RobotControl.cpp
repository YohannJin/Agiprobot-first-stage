...
/*
Describtion:
RobotControl uses the ur_rtde library with its rtde_control and rtde_receive classes to control and communicate with the robot
Visit their website for information on implemented methods:
...
std::vector<double> RobotControl::TransformBaseToToolFrame(std::vector<double> baseFramePoint)
{
	...
	// initiate matrices
	int BaseFrame[3][3] = {{1, 0, 0}, 
						   {0, 1, 0},
						   {0, 0, 1}}; // unit matrix
	std::vector<double> xToolFrameAxis = {-currentTCPPose.at(0) + xDircetionPoint.at(0), -currentTCPPose.at(1) + xDircetionPoint.at(1), -currentTCPPose.at(2) + xDircetionPoint.at(2)};
	std::vector<double> yToolFrameAxis = {-currentTCPPose.at(0) + yDircetionPoint.at(0), -currentTCPPose.at(1) + yDircetionPoint.at(1), -currentTCPPose.at(2) + yDircetionPoint.at(2)};
	std::vector<double> zToolFrameAxis = {-currentTCPPose.at(0) + zDircetionPoint.at(0), -currentTCPPose.at(1) + zDircetionPoint.at(1), -currentTCPPose.at(2) + zDircetionPoint.at(2)};
	...
	double detToolFrame = ToolFrame[0][2] * (ToolFrame[1][0] * ToolFrame[2][1] - ToolFrame[2][0] * ToolFrame[1][1]) +
	...
	invToolFrame[0][0] = (ToolFrame[1][1] * ToolFrame[2][2] - ToolFrame[2][1] * ToolFrame[1][2]) * invDetToolFrame;
	invToolFrame[0][1] = -(ToolFrame[0][1] * ToolFrame[2][2] - ToolFrame[2][1] * ToolFrame[0][2]) * invDetToolFrame;
	invToolFrame[0][2] = (ToolFrame[0][1] * ToolFrame[1][2] - ToolFrame[1][1] * ToolFrame[0][2]) * invDetToolFrame;
	invToolFrame[1][0] = -(ToolFrame[1][0] * ToolFrame[2][2] - ToolFrame[2][0] * ToolFrame[1][2]) * invDetToolFrame;
	invToolFrame[1][1] = (ToolFrame[0][0] * ToolFrame[2][2] - ToolFrame[2][0] * ToolFrame[0][2]) * invDetToolFrame;
	invToolFrame[1][2] = -(ToolFrame[0][0] * ToolFrame[1][2] - ToolFrame[1][0] * ToolFrame[0][2]) * invDetToolFrame;
	invToolFrame[2][0] = (ToolFrame[1][0] * ToolFrame[2][1] - ToolFrame[2][0] * ToolFrame[1][1]) * invDetToolFrame;
	invToolFrame[2][1] = -(ToolFrame[0][0] * ToolFrame[2][1] - ToolFrame[2][0] * ToolFrame[0][1]) * invDetToolFrame;
	invToolFrame[2][2] = (ToolFrame[0][0] * ToolFrame[1][1] - ToolFrame[1][0] * ToolFrame[0][1]) * invDetToolFrame;
...
{	
	...
	std::vector<double> normAxisAngle = {axisAngle[0]/magnitude, axisAngle[1]/magnitude, axisAngle[2]/magnitude};
...
int RobotControl::moveitPlanTo(std::vector<double> goalTP)
{
	...
	ROS_INFO_STREAM("Calculated goal Quaternions: " << myQuaternion[0] << ", "
													<< myQuaternion[1] << ", "
													<< myQuaternion[2] << ", "
													<< myQuaternion[3]);
	ROS_INFO_STREAM("Calculated goal Position: " << goalTP[0] << ", "
												 << goalTP[1] << ", "
												 << goalTP[2]);
...
{	
	...
	std::vector<double> normalVector = {offsetAlongY.at(0) - currentTCPPose.at(0), offsetAlongY.at(1) - currentTCPPose.at(1), offsetAlongY.at(2) - currentTCPPose.at(2)};
	double magnitudeOfNormalVector = sqrt(pow(normalVector.at(0), 2) + pow(normalVector.at(1), 2) + pow(normalVector.at(2), 2));
	std::vector<double> standardizedNormalVector = {normalVector.at(0) / magnitudeOfNormalVector, normalVector.at(1) / magnitudeOfNormalVector, normalVector.at(2) / magnitudeOfNormalVector};
...
void RobotControl::gotoTP(std::vector<double> goal, bool useMoveit, double velocity, double acceleration)
{
	if(useMoveit == true)
	{
	...
	{
		rtde_control->moveL(goal, velocity, acceleration);
	}
}
...
void RobotControl::gotoTP(Goal goal, bool useMoveit, double velocity, double acceleration)
{
	...
	}else
	{
		switch(goal)
		{
			case HOME:    		rtde_control->moveL(home_position_, velocity, acceleration);
                        		break;
        	case CLAMPDEVICE:   rtde_control->moveL(clampdevice_position_, velocity, acceleration);
                        		break;
        	case TOOLCHANGER:   rtde_control->moveL(toolchanger_position_, velocity, acceleration);
                        		break;
		}
	}
}
...
{	
	...
	switch(type) {
	case WRENCH_UP : 		forceFrame = rtde_receive->getActualTCPPose();
							taskFrame = { forceFrame.at(0), forceFrame.at(1), forceFrame.at(2), forceFrame.at(3), forceFrame.at(4), forceFrame.at(5)};
							selectionVector = {0, 1, 0, 0, 0, 0};
							wrench = {0, strength, 0, 0, 0, 0};
							limits = {2, 0.02, 2, 1, 1, 1};
    case WRENCH_DOWN : 		forceFrame = rtde_receive->getActualTCPPose();
							taskFrame = { forceFrame.at(0), forceFrame.at(1), forceFrame.at(2), forceFrame.at(3), forceFrame.at(4), forceFrame.at(5)};
							selectionVector = {1, 1, 1, 0, 0, 0};
							wrench = {0, -strength, 0, 0, 0, 0};
							limits = {0.05, 0.05, 0.05, 0.17, 0.17, 0.17};
             				break;
    case GETTOOLIN : 		taskFrame = {0, 0, 0, 0, 0, 0};
							selectionVector = {0, 0, 1, 0, 0, 0};
							wrench = {0, 0, -strength, 0, 0, 0};
							limits = {2, 2, 0.02, 1, 1, 1};
             				break;
	case GETTOOLOUT :		taskFrame = {0, 0, 0, 0, 0, 0};
							selectionVector = {1, 0, 0, 0, 0, 0};
							wrench = {strength, 0, 0, 0, 0, 0};
							limits = {0.02, 2, 2, 1, 1, 1};
							break;
	case EJECTTOOLIN :		taskFrame = {0, 0, 0, 0, 0, 0};
							selectionVector = {1, 0, 0, 0, 0, 0};
							wrench = {-strength, 0, 0, 0, 0, 0};
							limits = {0.02, 2, 2, 1, 1, 1};
							break;
	case EJECTTOOLOUT :		taskFrame = {0, 0, 0, 0, 0, 0};
							selectionVector = {0, 0, 1, 0, 0, 0};
							wrench = {0, 0, strength, 0, 0, 0};
							limits = {2, 2, 0.02, 1, 1, 1};
							break;
	...
	if (type == GETTOOLIN) vectorPosition = 2;
	if (type == EJECTTOOLIN) vectorPosition = 0;
	if (type == GETTOOLOUT) vectorPosition = 0;
	if (type == EJECTTOOLOUT) vectorPosition = 2;
	...
	{	
		...
		if(type == GETTOOLOUT)
		{
			while(goalReached == false)
			{
				...
				if(currentTCPPos[vectorPosition] > bottom_L1_[vectorPosition])
				{
					goalReached = true;
					rtde_control->forceModeStop();
				}
...
void RobotControl::changeTool(Slot fromSlot, Slot toSlot)
{
    ...
    switch(fromSlot)
    {
        case SLOT_1:    fromSlotPos = bottom_L1_;
                        break;
        case SLOT_2:    fromSlotPos = bottom_L2_;
                        break;
        case SLOT_3:    fromSlotPos = bottom_L3_;
                        break;
    }

    switch(toSlot)
    {
        case SLOT_1:    toSlotPos = top_L1_;
                        break;
        case SLOT_2:    toSlotPos = top_L2_;
                        break;
        case SLOT_3:    toSlotPos = top_L3_;
                        break;
    }
